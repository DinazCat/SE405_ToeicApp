...
const PostCard = ({
  ...
  const onLike = async () => {
    ...
    if (foundItem) {
      setIsLike(false);
      const newList = item.likes.filter(
  ...
  return (
    <View style={styles.Container}>
      ...
      <View>
        {item.postImg ? (
          item.postImg[0].type == 'img' ? (
            <Image
              source={{uri: item.postImg[0].uri}}
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate('ReadPDFScreen', {
                  link: item.postImg[0].uri,
                })
              }>
              <Image
                source={{
            ...
            <ScrollView style={{flexDirection: 'column'}}>
              {item.postImg[0].part == 'W1' && (
                <WriteP1QuestionForm
                  item={item.postImg[0].item}
                  part={item.postImg[0].part}
                  flag={'ReviewQuestion'}
                  check={item.postImg[0].check}
                />
              )}
              {(item.postImg[0].part == 'W2' ||
                item.postImg[0].part == 'W3') && (
