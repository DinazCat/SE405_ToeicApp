...
const PostScreen = ({navigation, route}) => {
  ...
  onUserPress = () =>
    navigation.navigate('profileScreen', {userId: postData.userId});
  onCommentPress = () =>
  ...
  const onLike = async () => {
    ...
    if (foundItem) {
      setIsLike(false);
      const newList = postData.likes.filter(
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.TextContainer}>
        ...
        <Text style={{fontStyle: 'italic', fontWeight: 'bold', color: '#333'}}>
          {'#' + postData?.hashtag}
        </Text>
            ...
            <View key={key}>
              {item.type == 'img' ? (
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    navigation.navigate('ReadPDFScreen', {link: item.uri})
                  }>
                  <Image
                    source={{
