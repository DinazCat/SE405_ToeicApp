...
const QuestionScreen = ({navigation, route}) => {
  ...
  const handleRecordComplete2 = (recordData, questionId) => {
    setRecordingsList(prevList => {
      const existingRecordIndex = prevList.findIndex(
  ...
  const onSubmit = async () => {
    if (part == 'S1' || part == 'S2' || part == 'S5') {
        ...
        const audioUploadPromises = recordsRef.current.map(async recordItem => {
          ...
          const audioData =
            recordItem.record !== null
              ? await RNFS.readFile(recordItem.record, 'base64')
              : null;
          const audioUrl =
            audioData !== null ? await Api.uploadAudio(audioData) : null;
          return {
            Qid: recordItem.Qid,
            record: audioUrl,
          };
      ...
      const audioUploadPromises = recordingsList.map(async recordItem => {
        // upload audio to storage
        const audioData0 =
          recordItem.record0 !== null
            ? await RNFS.readFile(recordItem.record0, 'base64')
            : null;
        const audioUrl0 = audioData0 ? await Api.uploadAudio(audioData0) : null;
        const audioData1 =
          recordItem.record1 !== null
            ? await RNFS.readFile(recordItem.record1, 'base64')
            : null;
        const audioUrl1 = audioData1 ? await Api.uploadAudio(audioData1) : null;
        const audioData2 =
          recordItem.record2 !== null
            ? await RNFS.readFile(recordItem.record2, 'base64')
            : null;
  ...
  useEffect(() => {
    scrollX.addListener(async ({value}) => {
      ...
      if (
        loading &&
        (part == 'L1' || part == 'L2' || part == 'L3' || part == 'L4')
      ) {
  ...
  return (
    <View style={styles.container}>
      <View style={AppStyle.viewstyle.component_upzone}>
        <TouchableOpacity style={{marginLeft: '2%'}} onPress={showAlert}>
              ...
              if (part == 'R2' || part == 'R3') {
                ...
                return (
                  <SpeakP1QuestionForm
                    ...
                    part={part}
                    flag={'QuestionScreen'}
                    onRecordComplete={(record, questionId) =>
                      handleRecordComplete(index, record, questionId)
                    }
                  />
                );
              } else if (part == 'S2') {
                ...
                return (
                  <WriteP1QuestionForm
                    ...
                    onAnswerChange={(answer, questionId) =>
                      handleAnswerChange(index, answer, questionId)
                    }
                  />
                );
              } else if (part == 'W2' || 'W3') {
