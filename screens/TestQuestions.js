...
const TestQuestions = ({navigation, route}) => {
  ...
  const timeFormat = totalSeconds => {
    ...
    let time = `${hours}:${minutes.toLocaleString('en-US', {
  ...
  const onSubmit = async () => {
    ...
    if (isMiniTest)
      ...
      for (let i = 0; i < questionList.length; i++) {
        if (
          questionList[i].part == 'L1' ||
          questionList[i].part == 'L2' ||
          questionList[i].part == 'R1'
        ) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < questionList.length; i++) {
      if (
        ...
        questionList[i].part == 'L3' ||
        questionList[i].part == 'L4' ||
        questionList[i].part == 'R2' ||
        questionList[i].part == 'R3'
      ) {
        list.push({
          Qid: questionList[i].Id,
