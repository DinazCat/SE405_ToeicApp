...
const Analysis = ({navigation, route}) => {
  ...
  const [ListenComplete, setListenComplete] = useState(0);
  const [ReadComplete, setReadComplete] = useState(0);
  ...
  const setLineChart2 = () => {
    ...
    line2[5] = format(OneDaysAgo, 'd/M/yyyy');
    line2[4] = format(TwoDaysAgo, 'd/M/yyyy');
    line2[3] = format(ThreeDaysAgo, 'd/M/yyyy');
    line2[2] = format(FourDaysAgo, 'd/M/yyyy');
    line2[1] = format(FiveDaysAgo, 'd/M/yyyy');
    line2[0] = format(sixDaysAgo, 'd/M/yyyy');
    setchartPr(line);
  ...
  const getPrHistory = async () => {
    firestore()
      ...
      .onSnapshot(querysnapshot => {
        ...
        setListenComplete(tongLis);
        setReadComplete(tongRead);
      });
  };
  const getRound = num => {
      ...
      .onSnapshot(querysnapshot => {
        ...
        querysnapshot.forEach(doc => {
          ...
          const score1 =
            ...
            5;
          const score2 =
            (data.Corrects[4] + data.Corrects[5] + data.Corrects[6]) * 5;
          if (score1 > highest) {
            highest = score1;
          }
          if (score2 > highest1) {
        ...
        // 6,25,39,30,30,16,54
        SetOverViewTest([
          getRound(list1[0] / (6 * count)),
          getRound(list1[1] / (25 * count)),
          getRound(list1[2] / (39 * count)),
          getRound(list1[3] / (30 * count)),
        ]);
        SetOverViewTest2([
          getRound(list1[4] / (30 * count)),
          getRound(list1[5] / (16 * count)),
          getRound(list1[6] / (54 * count)),
        ]);
      });
  };
  // useEffect(() => {
    ...
    await firestore()
      ...
      .then(querySnapshot => {
        ...
        if (data.AnalysisPractice) {
          const sumQty =
            convert(data?.AnalysisPractice?.L1?.Qty) +
            convert(data?.AnalysisPractice?.L2?.Qty) +
            convert(data?.AnalysisPractice?.L3?.Qty) +
            convert(data?.AnalysisPractice?.L4?.Qty);
          setListenQtyComplete(sumQty);
          const sumScore =
            convert(data?.AnalysisPractice?.L1?.Score) +
            convert(data?.AnalysisPractice?.L2?.Score) +
            convert(data?.AnalysisPractice?.L3?.Score) +
            convert(data?.AnalysisPractice?.L4?.Score);
          setListenScorePr(sumScore);
          // Reading

          const sumQty1 =
            convert(data?.AnalysisPractice?.R1?.Qty) +
            convert(data?.AnalysisPractice?.R2?.Qty) +
            convert(data?.AnalysisPractice?.R3?.Qty);
          setReadQtyComplete(sumQty1);
          const sumScore1 =
            (data?.AnalysisPractice?.R1?.Score || 0) +
            (data?.AnalysisPractice?.R2?.Score || 0) +
            (data?.AnalysisPractice?.R3?.Score || 0);
        ...
        .then(querySnapshot => {
          querySnapshot.forEach(doc => {
            ...
            const scoreL =
              ...
              data.Corrects[3];
            const scoreR =
              listR[6] + data.Corrects[4] + data.Corrects[5] + data.Corrects[6];
            if (time2[0] == line[6]) {
  ...
  const checkProgress = Part => {
    if (Part == 'L1') {
      ...
      if (rate < 0.5) {
        ...
        return {
          m: 'Your score is not very high in this part, please continue to practice more',
          c: 'orange',
        };
      } else if (rate > 0.7 && rate <= 0.9) {
        return {
          m: 'Congratulations, you are about to become a master in this part, keep trying',
          c: 'yellow',
        };
      } else if (rate > 0.9) {
        return {
          m: 'Congratulations, you are a master in this part, please maintain your current form',
          c: 'green',
        };
      }
    }
    if (Part == 'L2') {
      const rate1 = profileData
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        <CustomHeader Title={'Analysis'} navigation={navigation} />
        {!isLoading ? (
          <LottieView
          ...
          <>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <View style={{flex: 1}} />
              <TouchableOpacity
                style={[AppStyle.button.button1, {marginLeft: 5}]}
                onPress={() => {
                  setSelectedTabTest(0);
                }}>
              ...
              <LineChart
                data={{
                  ...
                  datasets: [
                    {
                      data:
                        selectedTabTest == 0 ? chartTestData : chartTestData1,
                    },
                  ],
                }}
                  ...
                  renderItem={({item, index}) => {
                    return (
                      <View style={styles.overviewBox}>
                        ...
                        <Text style={[styles.StorageText, {width: 100}]}>
                          correct:{item * 100}%
                        </Text>
                        <View style={{width: 100, flexDirection: 'row'}}>
                          <Text style={styles.StorageText}>
                            {checkOverView(item).mess}
                          </Text>
                          <Image
                            style={{width: 20, height: 20, marginLeft: 5}}
                            source={checkOverView(item).image}
                          />
                        </View>
                      </View>
                    );
            ...
            <View style={{flexDirection: 'row'}}>
              <Text style={AppStyle.textstyle.parttext}>
                Practice Analysis:
              </Text>
              <View style={{flex: 1}} />
              <TouchableOpacity
            ...
            <View
              ...
              <View style={styles.boxStorage}>
                ...
                <Text style={styles.StorageText}>
                  {selectedTab == 0 ? ListenComplete : ReadComplete}
                </Text>
              </View>
              {/* <View style={styles.boxStorage}>
            <Text style={[AppStyle.button.buttonText, {fontWeight: 400}]}>
            ...
            <Text style={styles.StorageText}>10</Text>
              ...
              <View style={styles.boxStorage}>
                ...
                <Text style={styles.StorageText}>
                  {selectedTab == 0
                    ? ListenScorePr + '/' + ListenQtyComplete
                    : ReadScorePr + '/' + ReadQtyComplete}
                </Text>
              </View>
            </View>
            {/* vẽ biểu đồ đường tỉ lệ đúng trong 7 ngày */}
            {chartPrData != [0, 0, 0, 0, 0, 0, 0] && (
              <View>
                <Text
                  style={[styles.StorageText, {marginLeft: 5, marginTop: 5}]}>
                  Correct ratio Line Chart
                </Text>
                <LineChart
                  data={{
            ...
            <View>
              <Text
                style={[styles.StorageText, {marginLeft: 5, marginTop: 10}]}>
