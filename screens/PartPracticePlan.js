...
const PartPracticePlan = ({navigation, route}) => {
  ...
  const [currentAssessmentScore, setCurrentAssessmentScore] = useState();
  ...
  useEffect(() => {
    socketServices.on(user.uid + 'PracticePlanChange', data => {
      ...
      if (data.PracticePhases[data.CurrentPhase.PhaseIndex].Test != undefined)
        ...
        setCurrentDays(data.PracticePhases[data.CurrentPhase.PhaseIndex].Days);
      setCurrentContent(
        data.PracticePhases[data.CurrentPhase.PhaseIndex].Content,
      );
      setCurrentTarget(
        data.PracticePhases[data.CurrentPhase.PhaseIndex].Target,
      );
      setCurrentAssessmentScore(
        data.PracticePhases[data.CurrentPhase.PhaseIndex].AssessmentScore || 0,
      );

      const Days = data.PracticePhases[data.CurrentPhase.PhaseIndex].Days;
      if (Days) {
        const totalCompleted = Days.reduce(
          (acc, day) => acc + day.CompletedQuestion,
          0,
        );
        const totalQuestions = Days.reduce(
          (acc, day) => acc + day.NumberofQuestions,
          0,
        );
  ...
  const onAssessmentPress = async item => {
  ...
  function PracticeCard({item}) {
    if (item.Day != currentDay)
      return (
        <View
          style={[AppStyle.viewstyle.row_start, {height: 60, marginTop: 10}]}>
      ...
      return (
        <>
          <View
            style={[
              styles.DayWrapper,
              {backgroundColor: PRIMARY_COLOR, marginLeft: '5%', marginTop: 10},
            ]}>
            <Text style={styles.DayText}>Day {item.Day}</Text>
          </View>
          <View
            ...
            <ImageBackground
              ...
              <View style={{marginTop: 10, marginLeft: 15}}>
                <Text
                  ...
                  {item.CompletedQuestion}/{item.NumberofQuestions}
                </Text>
              </View>
              <View
                style={{marginTop: 75, marginLeft: 15, flexDirection: 'row'}}>
                <View>
                  <Text
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              if (item.CompletedQuestion < item.NumberofQuestions) return;
              const questionList = await Api.getQuestion(6, 'ListenPart1');
      ...
      <>
        ...
        <View
          ...
          <ImageBackground
            ...
            style={{height: '100%', width: '100%'}}>
            <View
              style={{marginTop: 125, marginLeft: 15, flexDirection: 'row'}}>
              <View>
    ...
    <View style={styles.container}>
      <View style={AppStyle.viewstyle.component_upzone}>
        <TouchableOpacity
          ...
          onPress={() =>
            navigation.navigate('ChoosePracticePlan', {
              TargetLevel: PracticePlan.TargetLevel,
            })
          }>
          <Icon
          ...
          />
          <Text
            style={{color: PRIMARY_COLOR, fontWeight: 'bold', fontSize: 19}}>
            {PracticePlan.TargetLevel == 3
              ? '605'
              : PracticePlan.TargetLevel == 4
              ? '785'
              : '905'}
          </Text>
        </TouchableOpacity>
          ...
          <View>
            ...
            <Text style={{color: '#333', fontSize: 17}}>
              ...
              <Text style={{fontWeight: 'bold'}}>
                {currentAssessmentScore}/{currentTarget}
              </Text>
            </Text>
          </View>
        </View>
