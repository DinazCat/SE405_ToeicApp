...
const General = ({calendar, classId, start, finish}) => {
  ...
  const [isDrop, setIsDop] = useState(true)
             ...
             renderItem={({item, index}) => {
                      return (
                         <View style={{height:50, alignSelf:'center', flexDirection:'row', justifyContent:'space-evenly', alignItems:'center', backgroundColor:card_color, width:350}}>
                          <Text style={{color:'black', fontWeight:500, fontSize:16}}>{item.Date}</Text>
                          <Text style={{color:PRIMARY_COLOR, fontWeight:500, fontSize:15}}>{item.Time_start+ ' -> '+item.Time_finish}</Text>
                         </View>
                      )
             }}
           />
