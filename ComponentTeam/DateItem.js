...
const DateItem = ({item, files}) => {
    ...
    collectionRef.onSnapshot(
      ...
      querySnapshot => {
        ...
        querySnapshot.forEach(doc => {
          ...
          if (
            newDocumentData.classId == item.classId &&
            moment(newDocumentData.Date, 'DD/MM/YYYY').isSame(
  ...
  const getfile = () => {
    ...
    if (files) {
      for (let i = 0; i < files.length; i++) {
        ...
        if (
          moment(dateString, 'DD/MM/YYYY').isSame(
            moment(item.Date, 'DD/MM/YYYY'),
          )
        ) {
          list.push(files[i]);
  ...
  return (
    <View
      style={{width: screenWidth * 0.9, alignSelf: 'center', marginTop: 10}}>
